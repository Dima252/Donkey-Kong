...
bool game_config::new_game_load() {
    Board b;
    char last_key = 's';                                                     //Makes sure Marios initial state will be still
    int lives = 3, num_of_screen = 0, score = 0, iteration = 0;
    ...
    while (true) {
        start_game_load(b, last_key, lives, won, invalid_file, num_of_screen, score, fileNames[num_of_screen], iteration);  //Starts the game
...
bool game_config::new_game_save() {
    Board b;
    char last_key = 's';                                                     //Makes sure Marios initial state will be still
    int lives = 3, num_of_screen = 0,score = 0, iteration = 0;
    bool won = false, start = true, invalid_file = false;
    ...
    while (true) {
		start_game_save(b, last_key, lives, won, invalid_file, num_of_screen,score, fileNames[num_of_screen], iteration);  //Starts the game
...
void game_config::start_game_load(Board& b, char& last_key, int& lives, bool& won, bool& invalid_file, int num_of_screen, int& score, std::string filename,int& iteration) {
    //Mario
    Mario m;
    int jumps = 0;
    ...
    char key_pressed = 's', loadchar = ' ';

    b.load(GAME);
    if (!set_board(b, m, barrel, ghosts, Legend_x, Legend_y)) {
        invalid_file = true;
        return;
    }
    ...
    while (true) {   //loop of the game 
        ...
        if (steps.isNextStepOnIteration(iteration)) {
           loadchar = steps.popStep();
           handel_input_load(m, jumps, fall, on_ladder, key_pressed, hammer_found, barrel, ghosts, score, loadchar);
        }

        if (barrel_handling(b, barrel, barrel_timer, barrel_index, m) || fallDamage(m) || ghosts_handeling(b, ghosts, m)) {
            // check if the result is correct            
            if (results.popResult() != std::make_pair(iteration, Results::hitBomb)) {
...
void game_config::start_game_save(Board& b, char& last_key, int& lives, bool &won, bool &invalid_file, int num_of_screen,int &score, std::string filename, int& iteration) {
    //Mario
    Mario m;
    int jumps = 0 ;
    bool fall = false, on_ladder = false, hammer_found=false;
    ...
    while (true) {   //loop of the game 
		...
		saveKey = handel_input(m, jumps, fall, on_ladder, key_pressed, hammer_found, barrel, ghosts, score); //Handels input from the user
        if (m.correctInput(saveKey)) { steps.addStep(iteration, saveKey); }
...
bool game_config::set_board(Board& b, Mario& m, barrels barr[], std::vector<ghost>& ghosts, int &Legend_x, int &Legend_y) {
...
char game_config::handel_input(Mario& m, int& jumps, bool& fall, bool& on_ladder, char& Key_pressed,bool hammer_found, barrels barr[], std::vector<ghost>& ghosts, int& score) {
    if (_kbhit() && (jumps == 0) && fall == false) {
		...
		handleHammerAction(hammer_found, Key_pressed, m, barr, ghosts,score); //Handels the hammer action   
...
void game_config::handel_input_load(Mario& m, int& jumps, bool& fall, bool& on_ladder, char& Key_pressed, bool hammer_found, barrels barr[], std::vector<ghost>& ghosts, int& score, char loadchar) {
    Key_pressed = loadchar;  // Use the parameter directly
    game_pause(Key_pressed); // Check for pause 
...
void game_config::handleHammerAction(bool hammer_found, char Key_pressed,Mario& m, barrels barr[], std::vector<ghost>& ghosts, int& score) {
    if (hammer_found) {
...
bool game_config::ghosts_handeling(Board& b, std::vector<ghost>& ghosts, Mario& m) {
    for (int i = 0; i < ghosts.size(); i++) {
...
void game_config::reportResultError(const std::string& message, const std::string& filename, size_t iteration) {
    system("cls");
